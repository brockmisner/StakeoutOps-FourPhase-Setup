...
export function FleetPlanner(props: Props) {
  ...
  useEffect(() => {
    ...
    const hours = ((openTime?.start ?? now) - selectedStart) / 3600_000;
    timeline.current.scrollLeft = Math.max(0, Math.min(hours - 1, 23)) * Math.max(480, calendarWidth - 124) / zoom;
  }, [mode, selectedStart, now, openTime?.start, calendarWidth, zoom]);
  ...
  const attention = events.filter(e => e.attention || e.status === "failed").length;
  ...
  return <div className={styles.planner}>
    <section className={styles.metrics} aria-label="Planning summary">
      {[{ icon: Check, tone: "teal", value: ready ? `${completed} / ${events.length}` : "—", label: "Tasks complete", note: mode === "day" ? shortDate(date) : "Selected week" },
        ...
        { icon: Clock3, tone: "blue", value: !ready ? "—" : reliable && finish ? clock(finish, dateInZone(finish,timezone) !== date) : events.length && !unfinished.length ? "Complete" : "—", label: "Estimated finish", note: hasUnplaced ? "Resolve unplaced work" : "Current queue + buffers" },
    ...
    <section className={`${styles.outlook} ${!reliable || hasUnplaced ? styles.outlookWarning:""}`} aria-label="Capacity outlook"><span><CalendarDays size={23}/></span><div><strong>{!ready ? "Loading capacity outlook" : !reliable ? "Forecast needs attention" : hasUnplaced ? "Some tasks need a scheduling decision" : free ? "Room for the next task" : "The planning window is full"}</strong><p>{ready&&reliable&&free ? `Next 15-minute opening: Slot ${free.lane+1}, ${clock(free.start,true)}. Startup buffer included.` : "Forecast based on current queue and estimated durations."}</p></div><button onClick={()=>setSettings(!settings)}>Inspect forecast</button></section>
    <section className={styles.panel} aria-labelledby="client-progress-title"><div className={styles.panelHeading}><div><h2 id="client-progress-title">Client progress</h2><p>{mode === "day" ? "Daily" : "Weekly"} completion and dedicated devices, together.</p></div><span className={styles.clientCount}>{props.clients.length} clients</span></div><div className={styles.clientTableScroll}><table className={styles.clientTable}><thead><tr><th>Client</th><th>Phones</th><th>Phase</th><th>Completion</th><th>Attention</th></tr></thead><tbody>{props.clients.filter(c=>clientId === "all" || clientId === c.id).map(c=>{
      const list=events.filter(e=>e.clientId===c.id);const done=list.filter(e=>e.status==="succeeded").length;const assigned=props.phones.filter(p=>p.clientId===c.id);const count=list.filter(e=>e.attention).length;
      return <ClientProgress key={c.id} client={c.name} expanded={expandedClient===c.id} onClick={()=>setExpandedClient(expandedClient===c.id?null:c.id)} phones={assigned} phase={phaseFor(c.id)} events={list} done={done} attention={count} ready={ready} onSelect={selectEvent}/>;
    ...
    {selected&&<dialog ref={dialog} className={styles.drawer} onCancel={()=>setSelected(null)} onClick={e=>{if(e.target===e.currentTarget)setSelected(null);}}><div className={styles.drawerHeading}><h2>Task details</h2><button onClick={()=>setSelected(null)} aria-label="Close task details"><X size={20}/></button></div><h3>{selected.title}</h3><span className={styles.recordType}>{selected.projected ? "Projected occurrence · not dispatched" : "Recorded run"}</span><dl className={styles.details}>
...
function ClientProgress({client,phones,phase,events,done,attention,expanded,onClick,onSelect,ready}:{client:string;phones:CommandPhone[];phase:string;events:FleetEvent[];done:number;attention:number;expanded:boolean;onClick:()=>void;onSelect:(event:FleetEvent)=>void;ready:boolean}) {
  return <><tr><td><button className={styles.clientButton} onClick={onClick} aria-expanded={expanded}><span className={styles.initials}>{client.split(" ").slice(0,2).map(s=>s[0]).join("")}</span><strong>{client}</strong><ChevronRight size={15} style={{transform:expanded?"rotate(90deg)":undefined}}/></button></td><td>{phones.length}</td><td><span className={styles.phaseDot}/>{phase}</td><td><div className={styles.completion}><progress max={events.length||1} value={done}/><span>{ready?`${done} / ${events.length}`:"—"}</span></div></td><td>{ready?(attention||"—"):"—"}</td></tr>{expanded&&<tr><td colSpan={5}><div className={styles.phoneList}>{phones.length?phones.map(phone=>{
    const tasks=events.filter(e=>e.phoneId===phone.id);return <article key={phone.id}><div><Smartphone size={17}/><strong>{phone.name}</strong><span>{phone.status===1?"On":"Off"} · {tasks.filter(e=>e.status==="succeeded").length}/{tasks.length} complete</span></div><div>{tasks.map(task=><button key={task.id} onClick={()=>onSelect(task)}><span className={task.attention?styles.taskRisk:""}>{task.status.replaceAll("_"," ")}</span>{task.title}<ArrowUpRight size={13}/></button>)}</div></article>;
